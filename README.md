...
- 🕐 2차 스크럼 (02:20 ~ 03:20)
  - 현재진행상황 공유
  - 추후 진행상황 브리핑
    - Client : 11/09 부터 스토어 기능 연동 시작
    - POPO, BingSu :
      - 11/10 까지 API 기능 구현 완료(로컬)
      - 11/11 더미 데이터 추가 및 dev 연동확인
  - 문제점 공유
    - 따로 문제되는 사항 없었음.
    -
### 2021-11-09
- API 추가 구현
  - 18번, 19번, 21번 : 유저 리뷰 관련 API 작성 완료
  - 50, 51, 53, 54 번 : 스토어 관련 상품 및 리뷰 조회 API 작성 완료

- 클라이언트 피드백간 API를 구글 스프레드시트로 공유해야한다고 피드백. -> 조치 완료.

### 2021-11-10
- API 추가 구현 및 보완
  - 12번, 22번, 74번 API 구축 완료

- 2차 피드백
  - 고급 기능(외부 API 이용) API 개발 추천
  - 카카오 로그인 메서드 random함수(Math.random에 고찰)
    - Math.Random()은 극도의 많은 횟수로 실행하게 되면 중복되는 결과를 반환하게 된다. 따라서 더 나은 \메서드로 수정필요.
    - DTO 생성자에 대한 필요성(@SETTER를 쓰게 되면 다른 사람이 값을 변경할 수 있게되어 좋지 않다.)
  - Git 관련 branch 관리
    - 작은 단위의 기능으로 PullRequest 후 Merge하는게 필요.
  
### 2021-11-11
- API 추가 구현 및 보완
    - 20.1 20.2 구현(유저 리뷰 수정 API)
    - 52번 API 구현 (스토어 단일상품 조회 API)
- 세부적 validation 설정
- 결제시스템 카카오 PAY API 구축
- DEV서버 연동완료(90%)
